...
Public Class InventoryForm
    ...
    Private LowThreshold As Integer = 50
    Private MediumThreshold As Integer = 100
    Private HighThreshold As Integer = 200
    ...
    Private Sub StockStatus()
        For Each row As DataGridViewRow In gridItems.Rows
            If Not row.IsNewRow Then
                ...
                ElseIf qty < MediumThreshold Then
    ...
    Private Sub btnAdd_Click(sender As Object, e As EventArgs) Handles btnAdd.Click
        ...
        If dlg.ShowDialog = DialogResult.OK Then
            ...
            Using conn As New OleDbConnection(connStr)
                ...
                Dim cmd As New OleDbCommand("INSERT INTO Items (ItemID, ItemName, Quantity, Price, LastUpdated) VALUES (@itemID, @itemName, @quantity, @price, @lastUpdated)", conn)
                cmd.Parameters.AddWithValue("@itemID", dlg.ItemID)
                cmd.Parameters.AddWithValue("@itemName", dlg.ItemName)
                cmd.Parameters.AddWithValue("@quantity", dlg.Quantity)
                cmd.Parameters.AddWithValue("@price", dlg.Price)
                cmd.Parameters.Add("@LastUpdated", OleDbType.Date).Value = dlg.LastUpdated
    ...
    Private Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
        Dim dlg As New ItemUpdateForm With {
        .ItemID = gridItems.CurrentRow.Cells("ItemID").Value.ToString(),
        .ItemName = gridItems.CurrentRow.Cells("ItemName").Value.ToString(),
        .Quantity = Convert.ToInt32(gridItems.CurrentRow.Cells("Quantity").Value),
        .Price = Convert.ToDecimal(gridItems.CurrentRow.Cells("Price").Value),
        .LastUpdated = If(IsDBNull(gridItems.CurrentRow.Cells("LastUpdated").Value), DateTime.Now, Convert.ToDateTime(gridItems.CurrentRow.Cells("LastUpdated").Value))
        }
        ...
        If dlg.ShowDialog() = DialogResult.OK Then
            Using conn As New OleDbConnection(connStr)
                ...
                Dim cmd As New OleDbCommand("UPDATE Items SET ItemName=@itemName, Quantity=@quantity, Price=@price, LastUpdated=@lastUpdated WHERE ItemID=@itemID", conn)
                cmd.Parameters.AddWithValue("@itemName", dlg.ItemName)
                cmd.Parameters.AddWithValue("@quantity", dlg.Quantity)
                cmd.Parameters.AddWithValue("@price", dlg.Price)
                cmd.Parameters.Add("@LastUpdated", OleDbType.Date).Value = dlg.LastUpdated
                ...
                If rowsAffected = 0 Then
                    MessageBox.Show("No rows updated. Check ItemID value.", "Update Failed", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                End If
            End Using
            ...
            Using conn As New OleDbConnection("Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" & Application.StartupPath & "\Inventory.accdb")
                ...
                For Each row As DataRow In dt.Rows
                    ...
                    If exists = 0 Then
                        ...
                        insertCmd.Parameters.AddWithValue("@lastUpdated", row("LastUpdated"))
    ...
    Private Sub btnStockSettings_Click(sender As Object, e As EventArgs) Handles btnStockSettings.Click
        Dim dlg As New StockSettingsForm With {
        ...
        If dlg.ShowDialog() = DialogResult.OK Then
            LowThreshold = dlg.LowThreshold
            MediumThreshold = dlg.MediumThreshold
            HighThreshold = dlg.HighThreshold
            MessageBox.Show("Stock thresholds updated successfully.")
            StockStatus()
        End If
    End Sub
